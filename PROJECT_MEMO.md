...
3. 47개 아카이브 전부가 사이트 안에서 **도달 가능**하다 — ❌ 지금 6개만 링크됨(T3.2).
4. 30건 넘는 다이제스트를 스캔 가능한 밀도로 읽을 수 있다 — ❌ Phase 6 미착수(T3.1/T3.3).

즉 **남은 건 3번과 4번뿐이고, 둘 다 렌더링 작업이다.** LLM/수집 쪽은 손댈 게 없다.
...
| **T2.1** | 그라운딩 도메인 블록리스트 + primary-source 선호 | **설계 Phase 4 에서 유일하게 안 된 항목.** `gemini_grounding` 은 게시율 **86%**(전 소스 최고)라 여기 들어온 건 거의 그대로 페이지에 실린다. 2026-07-30 에 `buildfastwithai.com`·`unrot.co`(콘텐츠팜)가 통과했고 그 중 하나가 그날 중복까지 만들었다. 지금 유일하게 "싸고 품질에 직결"인 항목 | 코드 ~20줄, API 비용 0 |
| **T3.1** | `render.py`(1,069줄) → `templates/` + `static/digest.css` 분리 | Phase 6 의 **선행 조건**. Jinja 템플릿과 CSS가 전부 한 파일 문자열로 들어 있어서, 지금 디자인을 바꾸면 거대한 단일 파일에 머지하는 꼴이 된다. 검증은 `rerender.py` 로 47개 페이지 재생성 후 **의도한 변경 외 diff 0** | 리팩터 1회, API 비용 0 |
| **T3.2** | 아카이브 인덱스: 47개 중 6개만 링크됨 | `render_archive_index` 가 `digests[:6]` 만 `<a>` 로 만들고 나머지는 `+ 41 earlier digests` **죽은 텍스트**다. 6개월 백필의 산출물 대부분이 사이트에서 도달 불가 — 완성 정의 3번 | 작음(연/월 그룹핑 또는 전체 리스트) |
| **T3.3** | 밀도·시각 폴리시 패스 (설계 Phase 6) | 마지막에 하도록 설계에 박아둔 것. 이제 표시 필드가 전부 존재한다(`headline` · `Earlier:` 스레드 · `covered by N sources` · 소스헬스 배지) → **최종 콘텐츠 기준으로** 디자인 가능 | 디자인 1회 + 통합 |
| **T3.4** | 다이제스트 RSS/Atom 출력 (선택) | §5 파킹 항목. DB에 다 있어서 거의 공짜고, 나중에 리더/이메일 연동이 공짜가 된다. **없어도 완성 정의엔 안 걸린다** — 여력 있으면 | 작음 |
