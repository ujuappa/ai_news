...
- 2026-08-12: **홈 상단 크롬 정리.** 사용자가 스크린샷으로 지정: 마스트헤드·Wire·날짜 칩·Filters
  만 남기고 나머지는 뗀다. 큰 "AI Digest." 제목은 헤더 워드마크와 중복이고, 건수("17 stories
  today" / "618 in archive" / 카드 안 "17 stories")와 랭크("RANKED 01–17" / 티커 점수 /
  "Lead story · significance 0.70" / "Stories 02–04")는 본문 순서만으로 알 수 있다. Daily
  칩은 버튼이 아닌데 버튼처럼 보여서 같이 뺐다(주간 지면의 Weekly 칩도 같은 매크로라 같이
  사라짐). 지면 머리 h1 은 날짜 칩이 받는다. `rerender.py` 로 반영.

- 2026-08-12: **댓글 레일.** 사용자 스펙: 다이제스트 오른쪽 400px 레일, 행 클릭(제목 `<a>` 는
  원문)으로 연다, 한 단 답글, Public/Private, `?story=` 로 링크. **서버는 없다** — 스레드는
  `localStorage`(`ai-digest-comments`)에만 남고, parent_id 깊이 제한도 JS 저장 규칙이다.
  조판은 Mona Sans(스펙의 Archivo 는 08-07 에 폐기, 테스트가 되돌리는 걸 막음). 댓글을 쓰면
  그 기사를 자동 저장한다. `tests/test_comments.py`.
